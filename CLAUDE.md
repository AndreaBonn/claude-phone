...
- Choice buttons: `AskUserQuestion` does not exist in `-p` mode (measured on Claude Code 2.1.281); `prompts/telegram-bridge-system-v3.md` teaches Claude an `[[option: label]]` line syntax that `message_formatter.extract_choices` turns into buttons.
...
- Uploads: a document or photo sent to the bot is saved by `file_intake` in `<active project>/uploads/` (20 MB getFile limit, downloaded in memory, written only when complete). The sender's file name goes through `safe_filename` (basename, invisible characters and leading dots dropped); `write_unique` checks `uploads` against the `Sandbox` and creates with `O_EXCL|O_NOFOLLOW` under a directory fd, so nothing is ever replaced or followed. A caption starts a turn opened by `📎 File ricevuto: <path>`; with an album only the captioned item's turn exists, and it may start before the other files are saved. Video, audio and voice get an "unsupported" reply.
...
- "Approve always" grants (`ApprovalBroker.grants`) are checked after the sandbox, cover one exact Bash command or a whole non-Bash tool, live in memory only, and are revoked by `/new`, a profile switch or a restart.
- `/stop` and the ⏹️ button kill the claude process (SIGTERM, then SIGKILL); measured on Claude Code 2.1.281: after a SIGTERM mid-answer `--resume` keeps the same session and its context.
